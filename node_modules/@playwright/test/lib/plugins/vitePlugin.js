  ...
  return {
    ...
    setup: async (config, configDirectory, suite) => {
      ...
      try {
        ...
        (0, _utils.assert)(buildInfo.version === VERSION);
      ...
      const componentRegistry = new Map(); // 1. Re-parse changed tests and collect required components.

      const hasNewTests = await checkNewTests(suite, buildInfo, componentRegistry); // 2. Check if the set of required components has changed.

      const hasNewComponents = await checkNewComponents(buildInfo, componentRegistry); // 3. Check component sources.
      ...
      if (isAddressInfo(address)) process.env.PLAYWRIGHT_VITE_COMPONENTS_BASE_URL = `http://localhost:${address.port}/${relativeTemplateDir}/index.html`;
    },
    teardown: async () => {
  ...
  return {
    ...
    transform: async (content, id) => {
      ...
      if (!buildInfo.sources[file]) {
        ...
        } catch {// Silent if can't read the file.
        }
      ...
      if (!id.endsWith(`${relativeTemplateDir}/index.ts`) && !id.endsWith(`${relativeTemplateDir}/index.tsx`) && !id.endsWith(`${relativeTemplateDir}/index.js`)) return;
