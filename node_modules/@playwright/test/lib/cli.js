...
function restartWithExperimentalTsEsm(configFile) {
  const nodeVersion = +process.versions.node.split('.')[0]; // New experimental loader is only supported on Node 16+.

  if (nodeVersion < 16) return false;
  if (!configFile) return false;
  if (process.env.PW_DISABLE_TS_ESM) return false;
  if (process.env.PW_TS_ESM_ON) return false;
  if (!configFile.endsWith('.ts')) return false;
  if (!(0, _loader.fileIsModule)(configFile)) return false;
  const NODE_OPTIONS = (process.env.NODE_OPTIONS || '') + ` --experimental-loader=${_url.default.pathToFileURL(require.resolve('@playwright/test/lib/experimentalLoader')).toString()}`;
